...
from aiohttp import FormData, WebSocketError, WSMsgType
...
from arclet.edoves.main.interact.server_docker import (
    BaseDockerMetaComponent,
    BaseServerDocker,
    DockerBehavior,
)
from arclet.edoves.main.network import NetworkStatus
from arclet.edoves.main.utilles import DatetimeEncoder, IOStatus, error_check
    ...
    async def ws_connection(self) -> Union[AiohttpWSConnection, None]:
        ...
        while self.data.state in (NetworkStatus.CONNECTING, NetworkStatus.RETRYING):
            if retry_count >= self.io.protocol.current_scene.config.ensure_retries:
                self.logger.warning(
                    f"{self.io.protocol.current_scene.scene_name}: MAHServerDocker connect failed"
                )
            ...
            try:
                try:
                    ...
                    await self.quit()
                    self.logger.warning(
                        f"{self.io.protocol.current_scene.scene_name}: MAHServerDocker stopped"
                    )
                    await asyncio.sleep(5.0)
                    self.logger.info(
                        f"{self.io.protocol.current_scene.scene_name}: MAHServerDocker restarting..."
                    )
    ...
    async def session_fetch(self) -> Optional[DictMedium]:
        ...
        try:
            ...
            try:
                try:
                    self.logger.debug(
                        f"{self.io.protocol.current_scene.scene_name}: WSConnection trying ping..."
                    )
                    await ws_conn.ping()
                    self.data.state = NetworkStatus.CONNECTED
                except Exception as e:
                ...
                self.logger.warning(
                    f"{self.io.protocol.current_scene.scene_name}: WSConnection cancelled, stop"
                )
                self.data.state = IOStatus.CLOSE_WAIT
        else:
            if ws_message.type is WSMsgType.TEXT:
                ...
                try:
                    ...
                    return DictMedium().create(
                        self.io.protocol.current_scene.protagonist, raw_data, "DataReceived"
                    )
                except Exception as e:
                    self.logger.error(
                ...
                self.logger.warning(
                    f"{self.io.protocol.current_scene.scene_name}: server close WSConnection."
                )
                self.data.state = IOStatus.CLOSE_WAIT
                return
            elif ws_message.type is WSMsgType.CLOSED:
                self.logger.warning(
                    f"{self.io.protocol.current_scene.scene_name}: WSConnection has been closed."
                )
                ...
                self.logger.error(
                    f"{self.io.protocol.current_scene.scene_name}: WSConnection error: {ws_message.data}"
                )
                return
            else:
                self.logger.warning(

...
class BotStatusMeta(ParserMetadata):
    parser_targets = ["BotGroupPermissionChangeEvent", "BotMuteEvent", "BotUnmuteEvent"]


class BotRelationshipMeta(ParserMetadata):
    parser_targets = ["BotJoinGroupEvent", "BotLeaveEventActive", "BotLeaveEventKick", "RelationshipRemove"]
...
class BotRelationshipOperateBehavior(ParserBehavior):
    ...
    async def from_docker(self, protocol: MAHProtocol, data: DictMedium):
        ...
        if ev_type == "BotJoinGroupEvent":
            ...
            notice = Notice().create(protocol.current_scene.protagonist, data.content, ev_type)
            await protocol.screen.push(notice)
            await protocol.screen.broadcast("RelationshipSetup", relationship="Group")
            ...
            await protocol.screen.push(notice)
            await protocol.screen.broadcast("RelationshipTerminate", relationship="Group")
            ...
            notice.operator = operator
            await protocol.screen.push(notice)
            await protocol.screen.broadcast("RelationshipSevered", relationship="Group")
    ...
    async def to_docker(self, protocol: MAHProtocol, data: DictMedium):
        ...
        if relationship == "Group":
            protocol.current_scene.protagonist.relation["parents"].remove(
                target.identifier
            )  # 群组与bot的关系解除
            for mo in target.children:
                mo.relation["parents"].remove(target.identifier)  # 群组与群组成员的关系解除
                if len(mo.parents) == 0:
                    protocol.current_scene.monomers.remove(
                        mo.metadata.identifier
                    )  # 群组成员与bot的所有关系解除
